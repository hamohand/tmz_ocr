...
2. **Enrichissement du Corpus** : Utiliser nos scripts Python (ex: `enrich_corpus.py`, `reinforce_cedilla.py`, `reinforce_weak_chars.py`, `reinforce_uppercase.py`) pour multiplier par 3 les caractères spéciaux et par 5 les caractères très rares, et renforcer les majuscules (500 occurrences).
3. **Génération GT (avec Augmentation de Données)** : Le script `generate_gt.py` génère des images avec 9 transformations : bruit gaussien, poivre & sel, rotation, flou, contraste, luminosité, fonds texturés, tailles de police variables, et couleurs de texte variables. 
   *Note : Le modèle v4 s'est basé sur un corpus de 22 524 lignes, générant 22 523 paires d'entraînement.*
4. **Intégration de Dictionnaires (Wordlist)** : Création de listes de mots (78 203 mots uniques), fichiers de ponctuation et de nombres pour améliorer la reconnaissance linguistique.
...
3. **Apprentissage LSTM** : Il envoie vos images à la boucle, vérifie ce que le modèle devine, le compare à la vérité terrain, puis corrige les poids du réseau (Les "Iterations").
   ...
   *Commande utilisée pour le modèle v4 (2nd pass fine-tuning) :*
   ```bash
   ...
   ```
   *Note : Le modèle v4 utilise un apprentissage en 2ème passe, s'appuyant sur le modèle tmz_latn v3 plutôt que le modèle de base français, pour améliorer la reconnaissance de la langue.*

4. **Combinaison** : L'outil referme le réseau neuronal avec le nouvel unicharset dans le fichier final `tmz_latn.traineddata`.

## Métriques et Résultats (Modèles v3 et v4)
...
**Performances atteintes (v4) :**
- Total des itérations : 19 000
- Meilleur checkpoint trouvé à l'itération : 5 420
- **BCER : 0.989%**
- **BWER : 3.000%**
- Taille du modèle : 4.0 MB

Ces résultats démontrent que le modèle v4 est extrêmement performant, avec un BCER inférieur à 1%, et prêt pour un usage intensif.
